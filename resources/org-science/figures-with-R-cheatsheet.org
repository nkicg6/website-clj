...
You want the figures you make to have a standard look and feel to them. I typically use [[https://ggplot2.tidyverse.org/][ggplot2]] along with [[https://cran.r-project.org/web/packages/cowplot/vignettes/introduction.html][cowplot]], which provides nice defaults on top of ggplot for scientific plotting. If you just want clean plots and granular control over your styles, I'd recommend using =theme_set(theme_classic())= and then some of my code below to set the sizes of your plots. 
...
#+BEGIN_SRC R :session rsesh :results output :exports both
  ...
  theme_set(theme_classic()) # basic, clear plot setup
...
=theme_set(theme_classic())= sets a basic, clear plot style. 
=theme_and_axis_size= stores =ggplot2= standard size and legend options. This one is really important for having the same text size for your plots (this can also be set inside =theme_set()=, but here it allows different styles per plot should you need it). I add this to every plot, and if I want a particular option changed (i.e. I do want a legend) I can specifically add that below these options. 
