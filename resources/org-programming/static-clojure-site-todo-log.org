...
Great, I can access all the elements of that map as normal now.
 
The end goal will be to use the =:title= key in the map to rename the article (url or just the link title) and associate that with the =:date=. then I want to make a list of all non-index pages under a certain category (programming or science) and add
